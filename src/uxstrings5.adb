...
package body UXStrings is
   ...
   function To_Index (Position : Cursor) return Natural is (Position.Index);

   -----------
   -- First --
   -----------

   function First (Container : Unicode_Character_Access) return Cursor is
   begin
   ...
   function Last (Container : Unicode_Character_Access) return Cursor is
   begin
      if Container'Length = 0 then
         return No_Element;
      else
   ...
   type Iterator is new UXString_Iterator_Interfaces.Reversible_Iterator with record
      Container : Unicode_Character_Access;
      Index     : Natural;
   end record;
   ...
   begin
      if Object.Index = 0 then
         return First (Object.Container);
      else
         return (Object.Container, Object.Index);
      end if;
   end First;
   ...
   function Iterate (Container : UXString) return UXString_Iterator_Interfaces.Reversible_Iterator'Class is
     (Iterator'(Container => Container.Chars, Index => 0));

   function Iterate
     (Container : UXString; Start : Cursor) return UXString_Iterator_Interfaces.Reversible_Iterator'Class is
     (Iterator'(Container => Container.Chars, Index => Start.Index));

   function Empty (Capacity : Natural := 10) return UXString is
     (Ada.Finalization.Controlled with Chars => new Unicode_Character_Array (1 .. Capacity), Finalized => False);

   function New_String (First, Last : Positive) return UXString is
     (Ada.Finalization.Controlled with Chars => new Unicode_Character_Array (First .. Last), Finalized => False);
   ...
   procedure Free is new Ada.Unchecked_Deallocation (Unicode_Character_Array, Unicode_Character_Access);
   ...
   begin
      return (for all Item of Source.Chars.all => Unicode_Character'Pos (Item) < 16#80#);
   end Is_ASCII;
   ...
   begin
      return (for all Item of Source.Chars.all => Unicode_Character'Pos (Item) < 16#1_00#);
   end Is_Latin_1;
   ...
   begin
      return (for all Item of Source.Chars.all => Unicode_Character'Pos (Item) < 16#1_0000#);
   end Is_BMP;
   ...
   begin
      return (Ada.Finalization.Controlled with Chars => new Unicode_Character_Array'(Source), Finalized => False);
   end From_Unicode;
   ...
   begin
      return Encode (Source.Chars.all, Output_BOM);
   end To_UTF_8;
   ...
   is
   begin
      return Encode (Source.Chars.all, To_UTF_Encoding (Output_Scheme), Output_BOM);
   end To_UTF_16;
   ...
   begin
      return From_Unicode (Left.Chars.all & Right.Chars.all);
   end "&";
   ...
   begin
      return From_Unicode (Left.Chars.all & Right);
   end "&";
   ...
   begin
      return From_Unicode (Left & Right.Chars.all);
   end "&";
   ...
   begin
      return Count (Source.Chars.all, Set);
   end Count;
   ...
   begin
      return From_Unicode (Trim (Source.Chars.all, Side));
   end Trim;
   ...
   begin
      return From_Unicode (Trim (Source.Chars.all, Left, Right));
   end Trim;
   ...
   begin
      return From_Unicode (Unicode_Character_Array'(1 .. Left => Right));
   end "*";
   ...
   begin
      return From_Unicode (To_Lower (Item.Chars.all));
   end To_Lower;
   ...
   begin
      return From_Unicode (To_Upper (Item.Chars.all));
   end To_Upper;
   ...
   begin
      return From_Unicode (To_Basic (Item.Chars.all));
   end To_Basic;
